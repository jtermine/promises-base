...
namespace Termine.Promises.WithRedis
{
    public static class Extensions
    {
		...
		public static ICanExtendAnyFixedRelationship HarborFixedRelationship(this ICanExtendAnyHarborBaseType<IAmAHarborProperty> baseType, string name, string caption = "")
        {
			if (baseType.Relationships.ContainsKey(name))
			{
				var relationship = baseType.Relationships[name] as ICanExtendAnyFixedRelationship;

				if (relationship == null) throw new InvalidOperationException("Relationship already exists and ");

				return (ICanExtendAnyFixedRelationship)baseType.Relationships[name];
			}

			var fixedRelationship = new HarborFixedRelationship { Name = name, Caption = caption };

			baseType.Relationships.Add(name, fixedRelationship);

			return fixedRelationship;
		}

        public static ICanExtendAnyTemporalRelationship HarborTemporalRelationship(this ICanExtendAnyHarborBaseType<IAmAHarborProperty> baseType, string name, string caption = "")
        {
			if (baseType.Relationships.ContainsKey(name))
			{
				var relationship = baseType.Relationships[name] as ICanExtendAnyTemporalRelationship;

				if (relationship == null) throw new InvalidOperationException("Relationship already exists and ");

				return (ICanExtendAnyTemporalRelationship)baseType.Relationships[name];
			}

			var temporalRelationship = new HarborTemporalRelationship { Name = name, Caption = caption };

			baseType.Relationships.Add(name, temporalRelationship);
        ...
        {
            relationship.R.SingletonMode = EnumRelationship_SingletonMode.IsASingleton_Always;
        ...
        {
            relationship.R.SingletonMode = EnumRelationship_SingletonMode.IsASingleton_WhenActive;
        ...
        {
            relationship.R.SingletonMode = EnumRelationship_SingletonMode.NotASingleton;
